...
// Pure SVG arc — no emoji spinner issues
function Arc({ progress=0, color="#00c8f0" }) {
  const w=200, h=50, cx=w/2;
  const pct = progress/100;
  const px = 16 + pct*(w-32);
  const py = h-10 - Math.sin(pct*Math.PI)*22;
  return (
    <svg width={w} height={h} viewBox={`0 0 ${w} ${h}`} style={{ overflow:"visible", display:"block", margin:"0 auto" }}>
      {/* Dashed track */}
      <path d={`M 16 ${h-8} Q ${cx} ${h-46} ${w-16} ${h-8}`} fill="none" stroke="#2a4060" strokeWidth="2" strokeDasharray="5 4"/>
      {/* Progress fill */}
      {progress>0&&<path d={`M 16 ${h-8} Q ${cx} ${h-46} ${px} ${py}`} fill="none" stroke={color} strokeWidth="2.5" opacity="0.9"/>}
      {/* Start dot */}
      <circle cx="16" cy={h-8} r="5" fill="#0d1525" stroke={color} strokeWidth="2.5"/>
      {/* End dot */}
      <circle cx={w-16} cy={h-8} r="5" fill="#0d1525" stroke={color} strokeWidth="2.5"/>
      {/* Plane — SVG path, no emoji, always renders */}
      <g transform={`translate(${px},${py})`}>
        <circle r="11" fill="#0d1525" stroke={color} strokeWidth="1.5" opacity="0.9"/>
        <text x="0" y="1" fontSize="13" textAnchor="middle" dominantBaseline="middle" fill="white" style={{ fontFamily:"sans-serif" }}>✈</text>
      </g>
    </svg>
  );
}
...
function FlightsScreen({ onTap, onAdd, leaveState, setLeaveState, navIdx, setNavIdx }) {
  ...
  function renderFlightGroup(flights, showAll) {
    const visible = showAll ? flights : [flights[0]];
    return visible.map((f, i) => (
      <div key={f.id}>
        {/* Connection bridge BEFORE this flight (if it has conn data = it's a connecting leg) */}
        {f.conn && <ConnBridge conn={f.conn}/>}
        <FlightCard f={f} onTap={onTap}/>
      </div>
    ));
  }
  ...
  return (
    <div style={{ padding:"14px 14px 100px" }}>
      ...
      {renderFlightGroup(outbound, showOutboundAll)}

      {/* RETURN */}
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginTop:20, marginBottom:12 }}>
        <div style={{ fontSize:11, color:C.accent, fontFamily:C.mono, fontWeight:700, letterSpacing:2 }}>RETURN · APR 27</div>
        {returnFlights.length > 1 && (
          <div onClick={()=>setShowReturnAll(o=>!o)} style={{ fontSize:11, color:C.accent, cursor:"pointer", fontFamily:C.mono }}>
            {showReturnAll ? "Hide ▲" : `All ${returnFlights.length} flights ▼`}
          </div>
        )}
      </div>
      {renderFlightGroup(returnFlights, showReturnAll)}
